...
from utils.constants import SIMILARITY_FUNCTIONS, DIVERSITY_FEATURES


def get_feature_representations(feature_names, examples, trg_examples, vocab, word2vec=None, topic_vectorizer=None,
                                lda_model=None):
    """
    ...
    :param topic_vectorizer: the CountVectorizer object used to transform tokenized documents for LDA
    :param lda_model: the trained LDA model
    :param lowercase: lower-case the input examples for source and target domains
    :return: the feature representations of the examples as a 2d numpy array of shape (num_examples, num_features)
    """
    features = np.zeros((len(examples), len(feature_names)))

    # (copies entire data, lots of mem used)
    #if lowercase:
    ...
    # get the term distribution of the entire training data (a numpy array of shape (vocab_size,) )
    train_term_dist = utils.similarity_measures.get_term_dist(examples, vocab)

    # get the term distribution of each training example (a numpy array of shape (num_examples, vocab_size) )
    term_dists = np.array([utils.similarity_measures.get_term_dist([example], vocab) for example in examples])

    # get the term distribution of the target data (a numpy array of shape (vocab_size, ) )
    trg_term_dist = utils.similarity_measures.get_term_dist(trg_examples, vocab)
    ...
    if any(f_name.startswith('topic') for f_name in feature_names):
        ...
        topic_dists = utils.similarity_measures.get_topic_distributions(examples, topic_vectorizer, lda_model)
        trg_topic_dist = np.mean(utils.similarity_measures.get_topic_distributions(trg_examples, topic_vectorizer, lda_model),
                                 axis=0)
        print('Shape of topic distributions:', topic_dists.shape, 'shape of trg topic dist:', trg_topic_dist.shape)
    ...
    if any(f_name.startswith('word_embedding') for f_name in feature_names):
        print('Retrieving the word embedding representations of source and target data...')
        word_reprs = utils.similarity_measures.weighted_sum_of_embeddings(examples, vocab.word2id, word2vec, train_term_dist)
        trg_word_repr = np.mean(utils.similarity_measures.weighted_sum_of_embeddings(trg_examples, vocab.word2id, word2vec,
                                                                                     trg_term_dist), axis=0)
        print('Shape of word representations:', word_reprs.shape, 'shape of trg word repr:', trg_word_repr.shape)

    for i in range(len(examples)):
        for j, f_name in enumerate(feature_names):
            # check whether feature belongs to similarity-based features, diversity-based features, etc.
            if f_name.startswith('topic'):
                f = utils.similarity_measures.similarity_name2value(f_name.split('_')[1], topic_dists[i], trg_topic_dist)
            elif f_name.startswith('word_embedding'):
                f = utils.similarity_measures.similarity_name2value(f_name.split('_')[2], word_reprs[i], trg_word_repr)
            elif f_name in SIMILARITY_FUNCTIONS:
                f = utils.similarity_measures.similarity_name2value(f_name, term_dists[i], trg_term_dist)
    ...
    ## explicitly delete
    del train_term_dist, term_dists, trg_term_dist
    return features


def get_feature_names(feature_set_names):
    """
    Given a list of feature set names, return the list of all their feature names.
    :param feature_set_names: a list of feature set names, e.g. 'similarity', 'diversity', etc.
    ...
    if 'word_embedding_similarity' in feature_set_names:
        print('Using similarity-based features with word embedding representations...')
        # jensen-shannon, renyi, bhattacharyya do not work for word embeddings as they require positive probabilities
...
def quadratic_entropy(example, train_term_dist, word2id, word2vec):
    ...
    for word_1 in set(example):
        ...
        for word_2 in set(example):
            ...
            sim = utils.similarity_measures.cosine_similarity(vec_1, vec_2)
            summed += sim * p_1 * p_2
    return summed
...
def renyi_entropy(example, domain_term_dist, word2id):
    ...
    if summed == 0:
        # 0 if none of the words appear in the dictionary; set to a small constant == low prob instead
        summed = 0.0001
