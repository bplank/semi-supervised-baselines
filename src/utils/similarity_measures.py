"""
Methods for measuring domain similarity according to different metrics.
Metrics are currently used only based on term distributions, but other representations may be used.

Adapted version from https://github.com/sebastianruder/learn-similarity
"""
...
def jensen_shannon_divergence(repr1, repr2):
    ...
    As used in Wu, F., & Huang, Y. (2016). Sentiment Domain Adaptation with Multiple Sources. ACL.
    Remus, R. (2012). Domain adaptation using domain similarity-and domain complexity-based instance selection for
    cross-domain sentiment analysis. Data Mining Workshops (ICDMW), 2012 IEEE."""
...
def renyi_divergence(repr1, repr2, alpha=0.99):
    ...
    As used in Van Asch, V., & Daelemans, W. (2010). Using Domain Similarity for Performance Estimation. CL"""
...
def get_domain_term_dists(term_dist_path, domain2data, vocab, lowercase=True):
    ...
    :param term_dist_path: the path where the term distributions of the domains should be saved
    :param domain2data: the mapping of domains to (labeled_reviews, labels, unlabeled_reviews) tuples
    :param vocab: the Vocabulary object
    :param lowercase: lower-case the input data
    :return: a mapping of domains to their term distributions, i.e. a numpy array of shape (vocab_size,)
    """
    domain2term_dist = {}
        ...
        with open(term_dist_path, 'r') as f:
            for line in f:
                ...
                assert len(term_dist) == vocab.size,\
                    'Length of term dist for %s should be %d, is %d.' % (domain, vocab.size, len(term_dist))
                assert np.round(np.sum(term_dist), 6) == 1, 'Sum of term distribution is %.6f instead of 1.' \
                                                            'The vocabulary was likely created with a larger' \
                                                            'max_vocab_size.' % np.sum(term_dist)
                domain2term_dist[domain] = term_dist
        assert set(domain2term_dist.keys()) == set(domain2data.keys()),\
            'Term distributions are not saved for all domains: "%s" and "%s" are not equal.'\
            % (' '.join(domain2term_dist.keys()), ' '.join(domain2data.keys()))
...
def get_term_dist(docs, vocab, lowercase=True):
    ...
    :param docs: a list of tokenized documents; can also contain a single document
    :param vocab: the Vocabulary object
    :param lowercase: lower-case the input data
    :return: the term distribution of the input documents, i.e. a numpy array of shape (vocab_size,)
    """
    ...
    # normalize the absolute frequencies to obtain a relative frequency term distribution
    term_dist /= np.sum(term_dist)
    if np.isnan(np.sum(term_dist)):
        # the sum is nan if docs only contains one document and that document has no words in the vocabulary
        term_dist = np.zeros(vocab.size)
    return term_dist
...
def get_most_similar_domain(trg_domain, domain2term_dists, similarity_name='jensen-shannon'):
    """
    Given a target domain, retrieve the domain that is most similar to it according to some
    domain similarity measure (default: Jensen-Shannon divergence).
    ...
    :param similarity_name: a string indicating the name of the similarity measure used
...
def train_topic_model(examples, vocab, num_topics=50, num_iterations=2000, num_passes=10):
    """
    Trains an LDA topic model on the provided list of tokenised documents and returns the vectorizer used for the
    transformation and the trained LDA model.
    ...
    :param num_passes: the number of passes over the corpus that should be performed
    :return: the CountVectorizer used for transforming the corpus and the trained LDA topic model
    """
    # the text is already tokenized and pre-processed; we only need to transform it to vectors
    ...
    # the gensim LDA implementation requires a sparse corpus; we could also use sci-kit learn instead
    lda_corpus = gensim.matutils.Sparse2Corpus(lda_corpus, documents_columns=False)
    print('Training LDA model on data of all domains with %d topics, %d iterations, %d passes...'
          % (num_topics, num_iterations, num_passes))
    lda_model = gensim.models.LdaMulticore(lda_corpus, num_topics=num_topics, id2word=vocab.id2word,
...
def get_topic_distributions(examples, vectorizer, lda_model):
    ...
    :return: an array of shape (num_examples, num_topics) containing the topic distribution of each example
    """
    ...
    for doc in gensim_corpus:
        topic_representations.append([topic_prob for (topic_id, topic_prob)
                                      in lda_model.get_document_topics(doc, minimum_probability=0.)])
...
def load_word_vectors(file, vocab_word_vec_file, word2id, vector_size=300, header=False):
    """
    Loads word vectors from a text file, e.g. the one obtained from http://nlp.stanford.edu/projects/glove/.
    :param file: the file the word vectors should be loaded from
    :param vocab_word_vec_file: the file where the word embeddings in the vocabulary can be stored for faster retrieval
    :param word2id: the mapping of words to their ids in the vocabulary
    :param vector_size: the size of the word vectors
    :param header: whether the word vectors text file contains a header; default is False
    :return the loaded word vectors as a dictionary mapping each word to its numpy vector
    """
    word2vector = {}
...
def weighted_sum_of_embeddings(docs, word2id, word2vector, term_dist):
    """
    Get a weighted sum of embeddings representation for a list of documents belonging to one domain. The documents are
    represented as a list of ngrams. Also works if the list only contains a single document.
    ...
    """
    # the factor with which the word probability is smoothed, we empirically set this to the some value used in
    ...
    for doc in docs:
        ...
        for word in doc:
            if word in word2vector:
                ...
                # weight the vector with the smoothed inverse probability of the word
                doc_vector += np.sqrt(t / (term_dist[word2id[word]])) * vector
                word_vector_count += 1
        if word_vector_count == 0:
            # this might be because the review is in another language by accident; set count to 1 to avoid division by 0
            word_vector_count = 1
